...
- On both Claude Code and Codex, follow [Workspace selection](../SKILL.md#workspace-selection) for swarm workers and fan-out playbooks. Prefer Coder via the target project's local `/cloud-workspace` skill. Local worktrees are only for small changes verified without package installs. Pass remote access instructions explicitly; a local subagent does not automatically run inside Coder.
- Keep the rest of the policy unchanged. Pass file pointers not inlined context, give each worker an exclusive workspace or checkout when they write, review every subagent's diff yourself.
