...
Writer launches must originate outside the target checkout and acquire an exclusive [checkout lease](workflow-gates.md#writer-ownership). Foreground writers also use a tracked companion job and wait for its result. Both modes keep the lease until explicit handoff; inspect and release it before the parent edits, tests, or commits there. A writer brief must require background children to finish before returning. Read-only launches do not acquire a lease.
