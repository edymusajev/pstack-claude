...
## 0.10.0 - cross-vendor model runners

Forked from michael-denyer/pstack-claude at 0.9.27. The port collapsed upstream's Anthropic, OpenAI, and xAI panels to three Claude variants because Claude Code is single-vendor at the `Agent` tool; its defaults also named full model IDs that the tool rejects (it takes `sonnet`, `opus`, `haiku`, `fable` only, and no effort), which is why `interrogate` carried a "closest alias" fallback. This release names models as entries, `<slug>` or `<slug>@<effort>`, across two vendors, and gives each vendor a dispatch that actually applies the entry.

A Claude entry runs as a generated plugin subagent under `agents/`, one per distinct entry in `models.json`, whose frontmatter carries the full model ID and Claude Code's `effort` key (`claude-fable-5-1@xhigh` runs as `pstack:fable-5-1-xhigh`). The generator writes and prunes them behind a marker comment; `poteto-agent` and `comment-sicko` are untouched, and a name collision with a hand-written agent is an error. A Codex entry runs through `skills/poteto-mode/scripts/codex-run.sh`, a wrapper over the codex plugin's companion runtime (`openai-codex/codex` for Claude Code) that finds the newest installed plugin version, passes `--model` and `--effort` through, runs read-only unless `--write` is given, and prints Codex's final message on stdout so a panel fans out as parallel background `Bash` calls beside the `Agent` calls. `references/runners.md` in `poteto-mode` documents both forms and carries the generated runner table; every model-consuming skill's dispatch prose points at it instead of naming a `model` parameter.

Defaults follow upstream's role map one effort notch down: every role upstream ran on Fable 5.1 at max runs `claude-fable-5-1@xhigh`; the panel is `claude-fable-5-1@xhigh`, `gpt-6-astra@high`, `gpt-5.6-terra@xhigh`, `claude-opus-5@high`; the fast code and explorer roles upstream gave to grok run `gpt-5.6-terra@high`. Three roles carry `claudeOnly` in `models.json` because they depend on Claude Code's MCP servers, which a Codex run cannot see (`why investigators`, `why synthesizer`, `reflect judgment, divergent, synthesizer`); `validateModels` refuses a Codex entry there and the stamped sections say so. The stray-slug scan now covers `gpt-*` too. `setup-pstack` detects Codex models from `~/.codex/models_cache.json`, validates the entry grammar and the Claude-only rule, and writes a personal runner to `~/.claude/agents/` for a Claude entry the plugin does not ship. The Codex-runtime mapping in `codex-tools.md` passes `gpt-*` entries through and substitutes `claude-*` ones.

Verified: `bun test tests/` passes every generator, model, and runner test (the 11 `worktree-audit` failures predate the fork on this machine); `codex-run.sh --model gpt-5.4-mini --effort low` returned Codex's reply through the companion in nine seconds; a `claude -p --plugin-dir` session dispatched `pstack:opus-5-high` and got its reply back. Not verified: that the `effort` frontmatter changes the reasoning the runner does, which the docs state and nothing here can observe, and Terra writing candidates inside a worktree through `--write --cwd`.
