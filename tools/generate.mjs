...
//   fails by name.
//   plugins/pstack/models.json (the model policy: per-role entries across the
//   Claude and Codex vendors, the diverse panel, available slugs)
//     -> each model-consuming skill's "## Models" section
//     -> setup-pstack's override-sheet block and interrogate's reviewer table
//     -> the "## Runner table" section of poteto-mode/references/runners.md
//     -> the "## Model names" section of poteto-mode/references/codex-tools.md
//     -> one generated runner subagent per distinct Claude entry under
//        plugins/pstack/agents/ (marked; hand-written agents are never touched)
...
//   No other claude-* or gpt-* slug may appear in skill prose; the scan below fails on strays.
//
...
export function promptStub({ name, menu }) {
  return (
    ...
    `Invoke the \`${name}\` skill and follow it. Resolve Claude tool names, model entries, and ` +
...
// ---- Model entries ---------------------------------------------------------
//
// A model entry is "<slug>" or "<slug>@<effort>". The slug names a row of
// models.json's available list, which spans two vendors: Claude models run as
// Claude Code subagents, Codex models run through the codex plugin's companion
// script. The optional effort is one of the reasoning levels both runtimes
// share; an entry without one inherits the parent session's level. The same
// grammar serves models.json, every stamped Models section, and the user's
// override sheet, so a value round-trips between them unchanged.

export const EFFORT_LEVELS = ["low", "medium", "high", "xhigh", "max"];
export const ENTRY_ALIASES = ["inherit-parent", "auto"];
export const VENDORS = ["claude", "codex"];

export function parseEntry(entry, models) {
  const m = /^([a-z][a-z0-9.-]*[a-z0-9])(?:@([a-z]+))?$/.exec(entry);
  if (!m) throw new Error(`model entry "${entry}" is not <slug> or <slug>@<effort>`);
  const [, slug, effort] = m;
  const available = models.available.find((a) => a.slug === slug);
  if (!available) {
    throw new Error(`model entry "${entry}": slug "${slug}" is not in models.json's available list`);
  }
  if (effort !== undefined && !EFFORT_LEVELS.includes(effort)) {
    throw new Error(`model entry "${entry}": effort "${effort}" is not one of ${EFFORT_LEVELS.join(", ")}`);
  }
  return { entry, slug, effort: effort ?? null, vendor: available.vendor, label: available.label };
}

// Every distinct entry the role table names, in first-seen order.
export function distinctEntries(models) {
  const seen = new Map();
  for (const role of models.roles) {
    for (const entry of role.models) {
      if (!seen.has(entry)) seen.set(entry, parseEntry(entry, models));
    }
  }
  return [...seen.values()];
}
...
// ---- Runner subagents -------------------------------------------------------
//
// Claude Code's Agent tool takes only the four model aliases and no effort, so
// a Claude entry cannot be expressed at the call site. A subagent definition
// takes both (a full model ID plus an `effort` frontmatter key), so each
// distinct Claude entry becomes one generated agent file, named from the slug
// and effort so a dispatch reads as the entry it resolves:
// "claude-fable-5-1@xhigh" runs as subagent_type "pstack:fable-5-1-xhigh".

export function runnerName(parsed) {
  return parsed.slug.replace(/^claude-/, "") + (parsed.effort ? `-${parsed.effort}` : "");
}

export const RUNNER_MARKER =
  "<!-- generated by tools/generate.mjs from plugins/pstack/models.json; edit there, not here -->";

export function runnerAgent(parsed) {
  const effortText = parsed.effort
    ? `${parsed.label} at ${parsed.effort} effort`
    : `${parsed.label} at the session's effort level`;
  const front = [
    `name: ${runnerName(parsed)}`,
    `description: pstack runner, ${effortText}. Dispatched by pstack skills for roles configured as ` +
      `\`${parsed.entry}\`; not a general-purpose helper.`,
    `model: ${parsed.slug}`,
    ...(parsed.effort ? [`effort: ${parsed.effort}`] : []),
  ];
  return (
    `---\n${front.join("\n")}\n---\n\n${RUNNER_MARKER}\n\n# pstack runner: ${effortText}\n\n` +
    "You are a pstack runner: one fixed model and effort level that pstack skills dispatch for a configured role. " +
    "Follow the brief you were given exactly and return what it asks for, nothing more.\n\n" +
    "- A brief that writes code: read the `poteto-mode` skill's `SKILL.md` in full first, including its inline " +
    "Principles index, and work in its style. Stay inside the worktree and paths the brief names.\n" +
    "- A brief that reviews, judges, explores, or investigates: the brief's template governs. Do not edit files " +
    "unless the brief says to.\n" +
    "- Return file pointers and findings, not inlined dumps of what you read.\n"
  );
}

export function claudeRunners(models) {
  return distinctEntries(models).filter((p) => p.vendor === "claude");
}

// Write one agent file per Claude entry and remove generated files no entry
// needs. A hand-written agent (no marker) is never overwritten or removed; a
// name collision with one is an error rather than a silent clobber.
export function syncRunnerAgents(agentsDir, models, { log = console.log } = {}) {
  mkdirSync(agentsDir, { recursive: true });
  const expected = new Map(claudeRunners(models).map((p) => [`${runnerName(p)}.md`, runnerAgent(p)]));
  let stamped = 0;
  let removed = 0;
  for (const [file, next] of expected) {
    ...
    if (existsSync(path)) {
      ...
      if (!current.includes(RUNNER_MARKER)) {
        throw new Error(`agents/${file}: a hand-written agent has the name a generated runner needs; rename one of them`);
      }
    }
  ...
  for (const file of readdirSync(agentsDir).sort()) {
    ...
    const path = join(agentsDir, file);
    if (!readFileSync(path, "utf8").includes(RUNNER_MARKER)) continue;
    unlinkSync(path);
    removed += 1;
    log(`removed orphan: agents/${file}`);
  }
  return { stamped, removed, total: expected.size };
}

// The dispatch form for one entry, as the runner table and skill prose cite it.
export function dispatchFor(parsed) {
  if (parsed.vendor === "claude") return `\`subagent_type: "pstack:${runnerName(parsed)}"\``;
  return `\`../scripts/codex-run.sh --model ${parsed.slug}${parsed.effort ? ` --effort ${parsed.effort}` : ""}\``;
}

export function runnerTable(models) {
  const rows = distinctEntries(models).map((p) => {
    const runs = `${p.label}, ${p.effort ? `${p.effort} effort` : "session effort"}`;
    return `| ${code(p.entry)} | ${p.vendor} | ${runs} | ${dispatchFor(p)} |`;
  });
  return ["| Entry | Vendor | Runs as | Dispatch |", "|---|---|---|---|", ...rows].join("\n");
}
...
export const RUNNERS_FILE = "plugins/pstack/skills/poteto-mode/references/runners.md";
...
export function regions(models) {
  ...
  return [
      ...
      })),
    {
      ...
      name: "reviewer table",
      locate: tableRows("| Reviewer | Default entry |", "| Reviewer "),
    ...
    {
      file: RUNNERS_FILE,
      name: "Runner table section",
      locate: section("Runner table"),
      render: () => blankPadded(runnerTable(models)),
    },
  ];
}
...
// A role's "models" is a list of entries or the string "panel", which resolves
// to the shared diverse-model panel so the panel is written once.
...
export function validateModels(models) {
  for (const a of models.available) {
    if (!VENDORS.includes(a.vendor)) {
      throw new Error(`models.json: ${a.slug} has vendor "${a.vendor}"; expected one of ${VENDORS.join(", ")}`);
    }
  }
  const labels = new Set();
  for (const role of models.roles) {
    if (labels.has(role.role)) throw new Error(`models.json: role "${role.role}" appears twice`);
...
export function loadModels() {
  return validateModels(resolveModels(JSON.parse(readFileSync(join(repo, "plugins/pstack/models.json"), "utf8"))));
}
...
export function modelsSection(roles) {
  ...
  return (
    ...
    "An entry reads `<slug>@<effort>` and names a Claude or Codex model; dispatch each through the " +
    "[runner table](../poteto-mode/references/runners.md).\n\n" +
    bullets
  );
}
...
export function overrideSheetBlock(models) {
  ...
  return (
    ...
    "An entry is `<slug>` or `<slug>@<effort>`: a Claude model runs as the matching `pstack:` runner subagent, " +
    "a Codex model runs through the codex plugin, and a missing effort inherits the session's level. " +
    "A value of `inherit-parent` or `auto` runs that role on the parent session's model and effort " +
    "(dispatch `general-purpose`, or `pstack:poteto-agent` for a code-writing brief, with no `model`); " +
...
export function codexModelNamesSection(models) {
  const strongest = models.roles.filter(
    (r) => r.models.length === 1 && parseEntry(r.models[0], models).slug === models.codex.strongestClaude,
  );
  return (
    "Skills name cross-vendor defaults. Each entry reads `<slug>@<effort>` and names a Claude model or a Codex " +
    "model; each model-consuming skill lists its own in a Models section, which Claude Code resolves through " +
    "[runners.md](runners.md). On Codex every subagent is a `spawn_agent`, so:\n\n" +
    "- Codex entries (`gpt-*`) resolve directly: the slug is the model and the `@effort` suffix is the reasoning effort.\n" +
    "- Claude entries (`claude-*`) do not resolve on Codex. Substitute a Codex model: for example " +
    `${code(models.codex.everydaySubstitute)} for everyday roles and ${code(models.codex.strongestSubstitute)} ` +
    "for the roles that default to the strongest Claude model " +
    `(${strongest.map((r) => code(r.role)).join(", ")}). Keep the diverse-model panels (\`arena\`, \`architect\`, ` +
    "`interrogate`, `reflect`) on distinct Codex models; if only one model family is reachable, vary reasoning " +
    "effort and note in the verdict that diversity was reduced.\n" +
    "- The generated runner subagents and `../scripts/codex-run.sh` are Claude Code plumbing; `spawn_agent` " +
    "replaces both.\n\n" +
    "`/setup-pstack` writes the configured entry list. On Codex, set every entry to a Codex slug."
  );
}

// After stamping, no model slug of either vendor may survive in skill prose
// outside the regions the generator owns in that file.
const SLUG_RE = /(?:claude-(?:opus|fable|sonnet|haiku)|gpt-[5-9])[0-9a-z.-]*/;
...
function main() {
  ...
  const runners = syncRunnerAgents(join(repo, "plugins/pstack/agents"), models);
  if (runners.stamped === 0 && runners.removed === 0) {
    console.log(`ok: ${runners.total} runner agents current`);
  }
